...
: _wge-packet-header ( srcip )
    2>r
    ETH.IP.UDP.WGE.REPLYTO.PORT packet@ d# 1627
    d# 2 ETH.IP.UDP.WGE.REPLYTO.IP enc-offset enc@n
    2r>
    d# 2 ETH.IP.UDP.WGE.REPLYTO.IP enc-offset enc@n arp-lookup
...
: handle-sensorselect
    ...
    ETH.IP.UDP.WGE.SENSORSELECT.INDEX enc-offset enc-c@
    cells sensors + !
    send-status
;
